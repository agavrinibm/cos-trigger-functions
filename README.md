...
1. Разверните приложение:
    ...
    ibmcloud cf push
    ```
    Если по какой-то причине Cloud Foundry развернута в другом регионе (в этом случае вы увидите ошибку "No CF API endpoint set"), вы можете переключиться на другой регион с помощью команды `ibmcloud target -r`, например:
    ```
